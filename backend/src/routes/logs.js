...
router.get('/', (req, res) => {
  ...
  const logs = db.prepare(`
    SELECT dl.id, dl.food_id, dl.servings, dl.portion_multiplier, dl.timestamp,
           f.name, f.serving_description, f.calories, f.fat_g, f.protein_g, f.carbs_g,
           f.image_url, f.emoji
    FROM daily_logs dl
...
router.get('/history', (req, res) => {
  ...
  const history = db.prepare(`
    SELECT dl.date,
           SUM(f.calories   * dl.servings * dl.portion_multiplier) as calories,
           SUM(f.fat_g      * dl.servings * dl.portion_multiplier) as fat_g,
           SUM(f.protein_g  * dl.servings * dl.portion_multiplier) as protein_g,
           SUM(f.carbs_g    * dl.servings * dl.portion_multiplier) as carbs_g,
...
// Update portion multiplier for an existing log entry (edit mode)
router.patch('/:id', (req, res) => {
  ...
  db.prepare('UPDATE daily_logs SET portion_multiplier = ? WHERE id = ?').run(multiplier, req.params.id);
